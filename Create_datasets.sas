...
%MACRO ALLDS;
	...
	proc fedsql %IF "&SYSVER." = "V.03.05" %THEN  sessref=mySession ;;
		create table dussmann.DS_GARE_PUNTEGGI    as
			select
				A.gpu_cod_gara,
				A.gpu_progr_ubi,
	...
	proc fedsql %IF "&SYSVER." = "V.03.05" %THEN sessref=mySession;;
		create table dussmann.ds_gare_dussmann     as
			select distinct
				A.gga_cod_gara,
		...
		case
			when B.gub_importo=0 then A.gga_tm_importo 
			else B.gub_importo
			end,
	...
	proc fedsql %IF "&SYSVER." = "V.03.05" %THEN sessref=mySession;;
		create table dussmann.ds_gare_no_dusmann    as
			select distinct 
				A.gga_cod_gara,
				A.gga_scad_iscrizione,
				C.gcl_nome,
